module.exports = (input) => {
  const createTree = (parent, nodes) => {
    let children = [];
    nodes.map((n, i) => { // change to reduce on []?
      let m = n.split('/'), j = m.indexOf(parent);
      if (j >= 0) {
        nodes.splice(i, 1);
        m.splice(j, 1);
        children.push({parent: +parent, value: +m[0], children: createTree(m[0], nodes)});
        nodes.unshift(n);
      }
    });
  ...
  const getMaxStrength = (node) => {
    ...
    return node.value + node.parent + Math.max.apply(null, node.children.map(c => {

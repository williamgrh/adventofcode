module.exports = (input) => {
  ...
  const execute = (p1, p2) => { // p1 is calling program, p2 is other program
    ...
    if (inst[0] === 'jgz' && (parseInt(inst[1], 10) > 0 || p1.r[inst[1]] > 0)) p1.pos = p1.pos - 1 + y;
    if (inst[0] === 'snd') p1.numSent++, p1.s.push(parseInt(inst[1], 10) || p1.r[inst[1]]);
    if (inst[0] === 'rcv') {
  ...
  while (!(input[A.pos][0] === 'rcv' && B.s.length === 0 && input[B.pos][0] === 'rcv' && A.s.length === 0)) {

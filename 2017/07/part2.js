...
module.exports = (input) => {
  ...
  const getWeight = (node) =>
    node.weight +
    node.children.reduce((sum, node) => sum + getWeight(nodes[node]), 0);
  const findUnbalancedChildAndDiff = (node) => {
    ...
    const diff =
      parseInt(Object.values(_.pickBy(dict, (v, k) => k !== "1"))[0], 10) -
      parseInt(dict["1"], 10);
  ...
  while (true) {
    ...
    if (
      child !== -1 &&
      nodes[nodes[node].children[child]].children.length > 0
    ) {

...
module.exports = (input) => {
  ...
  const getPermutations = (img) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // vertical
      rotations.push(rotations[i].split('/').map(r => r.split('').reverse().join('')).join('/'));
    }
  ...
  for (let iter = 0; iter < 5; iter++) {
    let img = image[0].split('/'), l = img.length;
    console.log(l);
    if (l % 2 === 0) {
      let rows = [];
      for (let r = 0; r < l; r++) rows.push([img[r].slice(0, l/2), img[r].slice(l/2)]);
      img = ['', '', '', ''];
      console.log(rows);
      return;
